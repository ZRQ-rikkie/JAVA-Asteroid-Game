...
//    private Font fontLarge = Font.font("Lucida Console", FontWeight.BOLD, 40);
   ...
   Background background = new Background(backgroundImage);
	...
	public void startGame(Stage menuStage, Stage stage, Menu menu) throws Exception {
	    ...
	    ship.lives = 3;
	    Ship ship = new Ship(PANE_WIDTH / 2,PANE_HEIGHT / 2);
		...
		ship.getCharacter().setFill(Color.BLACK);
		...
		Level level = new Level(gameText, font, fontSmall, asteroids, ship, pressedKeys,  bullets);
...
//		Map<KeyCode, Boolean> pressedKeys = new HashMap<>(); // keys are mapped to values, 
		...
		//We use the atomic integer to keep track of the score in the game 
		//The reason for using AtomicInteger is to have access to its getter and setter methods for automatically updating the scores
		...
		final int ALIEN_SPAWN_INTERVAL = 5;
		...
		new AnimationTimer() {
			...
			public void handle(long nanotime) {
				...
				//logic for spawning the alien
				//we have an alien flag so that only one alien can spawn per round
				if (currentLevel % 1 == 0 && !level.getAlienFlag()) {
				    long currentTime = System.nanoTime();
				    if (currentTime - alienSpawnTimer.get() >= ALIEN_SPAWN_INTERVAL * 1e9) {
				...
				asteroids.forEach(asteroid -> {
					// Adding splitting asteroids for when the player hits it
				...
				if (pressedKeys.getOrDefault(KeyCode.SPACE, false) && level.getCanShoot() && System.nanoTime()-shootTimer >= 3e8 && bullets.size() < 5) {
				...
				//all asteroids and aliens must be destroyed --> levelstarted flag must be true 
				//the flag is false during the game but is set to true 
				if (asteroids.isEmpty() && !level.getLevelStarted() && aliens.isEmpty()) {
					  level.setCanShoot(false); //set the canShoot flag to false so ship stops shooting at level screen
					  ...
					  level.levelScreen(rnd, pane);
		              alienSpawnTimer.set(System.nanoTime()); 
		              //ensures that the alien will spawn 10 seconds after the next round starts
	...
	//method to clear all objects from the screen 
