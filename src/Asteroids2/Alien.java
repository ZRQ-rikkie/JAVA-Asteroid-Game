...
public class Alien extends Character {
    ...
    public void move() {
        ...
        if (getCharacter().getTranslateX() > Asteroids.PANE_WIDTH) {
            getCharacter().setTranslateX(0);
        } else if (getCharacter().getTranslateX() < 0) {
            getCharacter().setTranslateX(Asteroids.PANE_WIDTH);
        }

        if (getCharacter().getTranslateY() > Asteroids.PANE_HEIGHT) {
            getCharacter().setTranslateY(0);
        } else if (getCharacter().getTranslateY() < 0) {
            getCharacter().setTranslateY(Asteroids.PANE_HEIGHT);
        }
    }
